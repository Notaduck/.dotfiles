        ...
        Snacks.picker.files {
            ...
            sorter = function(curr, next, debug)
                ...
                local function get_service_base(file)
                    ...
                    return base:gsub("Interface%.ts$", ""):gsub("Logger%.ts$", ""):gsub("%.test%.ts$", ""):gsub(
                        "%.spec%.ts$", ""):gsub("%.ts$", "")
                end

                -- Helper function to get file priority
                local function get_file_priority(file)
                    if file:match("/interfaces/") or file:match("Interface%.ts$") then
                        return 2
                    end
                    if file:match("Logger%.ts$") then
                        return 3
                    end
