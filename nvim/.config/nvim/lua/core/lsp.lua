...
vim.diagnostic.config({
  ...
  signs = {
    ...
    linehl = { [vim.diagnostic.severity.ERROR] = "DiagnosticSignError" },
    numhl  = {
...
local function try_attach_lsp(args)
  ...
  if filepath == "" or filepath:match("^%a+://") then return end
  ...
  for _, client in ipairs(clients) do
    ...
    if not should_attach then
      if (ft == "typescript" or ft == "javascript" or ft == "typescriptreact" or ft == "javascriptreact")
        and (client.name == "vtsls" or client.name == "tsserver")
        or (ft == "lua" and client.name == "lua_ls")
        or (ft == "python" and client.name == "pyright")
        or ((ft == "yaml" or ft == "yml") and client.name == "yamlls")
        or (ft == "go" and client.name == "gopls")
        or ((ft == "json" or ft == "jsonc") and client.name == "jsonls")
        or (ft == "dockerfile" and client.name == "dockerls")
      then
        should_attach = true
      end
    end
...
vim.api.nvim_create_autocmd({ "BufReadPost", "BufNewFile" }, { callback = try_attach_lsp })
